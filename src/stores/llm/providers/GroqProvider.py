from ..LLMInterface import TextGenerationInterface
from ..LLMEnums import OpenAIEnums
from groq import Groq
import logging


class GroqProvider(TextGenerationInterface):
    ...
    def generate_text(self, prompt: str, chat_history:list=[], max_output_tokens: int=None, temperature: float=None):
        if not self.client:
            self.logger.error("Groq client isn't set")
            return None
        
        if not self.generation_model_id:
            self.logger.error("Generation id for Groq isn't set")
            return None
